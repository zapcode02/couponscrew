...
import {
  ...
  ArrowRight, Dumbbell, Package, Star, RefreshCw, Lock, ExternalLink
} from 'lucide-react';
...
const PRODUCTS = [
  {
    id: 'pulgos-pushup-board',
    name: "PulGos 15-in-1 Pushup Board for Men & Women",
    brand: 'PulGos',
    image: 'https://res.cloudinary.com/dqjlffxja/image/upload/v1783162810/81RW0nGbEZL._SL1500__u4abvp.jpg',
    dealPrice: 399,
    originalPrice: 999,
    discountPercent: 60,
    affiliateUrl: 'https://amzn.to/4uUuG7b',
    slug: '/products/pulgos-15-in-1-pushup-board',
    isPopular: true,
  },
];
...
export default function ExerciseFitnessCategory() {
  ...
  const processed = useMemo(() => {
    let r = [...PRODUCTS];
    if (isFilterActive) r = r.filter(p => p.discountPercent >= 50);
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-12 gap-10 items-center text-left">
          <div className="md:col-span-7 flex flex-col items-start">
            <div className="inline-flex items-center gap-2 text-sm text-[#4A4A6A]">
              ...
              <ChevronRight size={14} className="text-gray-300 stroke-[2.5]" />
              <span className="text-[#FF5722] font-semibold">Exercise & Fitness</span>
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3 w-full">
              <div className="bg-white border border-[#E8E8F0] rounded-2xl px-4 py-3 flex items-center gap-3 shadow-[0_2px_8px_rgba(232,232,240,0.4)]">
                <div className="bg-[#FFF2ED] p-2.5 rounded-xl text-[#FF5722] shrink-0"><Dumbbell size={18} /></div>
                <div><div className="font-extrabold text-sm text-[#1A1A2E] leading-tight">1</div><div className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-1">Product</div></div>
              </div>
              <div className="bg-white border border-[#E8E8F0] rounded-2xl px-4 py-3 flex items-center gap-3 shadow-[0_2px_8px_rgba(232,232,240,0.4)]">
                <div className="bg-[#FFF3F0] p-2.5 rounded-xl text-[#FF5722] shrink-0"><Tag size={18} /></div>
                <div><div className="font-extrabold text-sm text-[#1A1A2E] leading-tight">1</div><div className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-1">Active Deal</div></div>
              </div>
              <div className="bg-white border border-[#E8E8F0] rounded-2xl px-4 py-3 flex items-center gap-3 shadow-[0_2px_8px_rgba(232,232,240,0.4)]">
          ...
          <div className="md:col-span-5 flex items-center justify-center relative w-full">
            <div className="relative w-full h-[280px] rounded-3xl overflow-hidden shadow-2xl">
              <img src="https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=800&q=80" alt="Exercise & Fitness" className="w-full h-full object-cover rounded-3xl hover:scale-105 transition-transform duration-700" referrerPolicy="no-referrer" />
              ...
              <div className="absolute bottom-4 right-4 bg-white rounded-2xl p-5 shadow-2xl min-w-[180px] border border-[#E8E8F0]/40 z-10 text-left">
                ...
                <div className="text-xl font-black text-[#1A1A2E] tracking-tight mt-0.5">FITNESS</div>
                <div className="text-xs text-[#4A4A6A] mt-1.5 font-semibold">Up to</div>
                <div className="text-3xl font-black text-[#FF5722] leading-none mt-0.5 tracking-tight">60% OFF</div>
                <a href="https://amzn.to/4uUuG7b" target="_blank" rel="noopener noreferrer nofollow sponsored" className="bg-[#FF5722] hover:bg-orange-600 text-white px-5 py-2.5 rounded-full text-xs font-bold mt-4 block w-full text-center transition-all duration-200 shadow-md">Shop Now</a>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-8 items-start">
          <div className="flex-1 w-full text-left">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3 pb-4 border-b border-[#E8E8F0]">
              <div>
                <h2 className="font-bold text-xl text-[#1A1A2E] leading-none">Exercise & Fitness Products ({PRODUCTS.length})</h2>
              ...
              <div className="flex items-center gap-2.5 shrink-0 select-none">
                ...
                <button onClick={() => setIsFilterActive(p => !p)} className={`border rounded-xl px-4 py-2 text-sm flex items-center gap-2 cursor-pointer transition ${isFilterActive ? 'border-[#5B4FBE] bg-[#F0EEFF] text-[#5B4FBE] font-bold' : 'border-[#E8E8F0] bg-white text-[#4A4A6A] hover:border-[#5B4FBE]'}`}>
                  ...
                  <span>{isFilterActive ? 'Filter: On' : 'Filter'}</span>
                </button>
              </div>
            </div>
          ...
          <aside className="w-full lg:w-72 flex-shrink-0 space-y-5 lg:sticky lg:top-24 text-left">
            ...
            <div className="bg-white rounded-2xl border border-[#E8E8F0] p-5 shadow-[0_2px_12px_rgba(232,232,240,0.3)]">
              ...
              <div className="space-y-3.5">
                {PRODUCTS.map((p, i) => {
                  const rankBg = i === 0 ? 'bg-[#FFD700] text-[#1A1A2E]' : 'bg-[#F0EEFF] text-[#5B4FBE]';
                  return (
                    <div key={p.id} className="flex items-center gap-3 cursor-pointer group">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 ${rankBg}`}>{i + 1}</div>
              ...
              <Link href="/products" className="text-xs font-bold text-[#5B4FBE] hover:underline inline-flex items-center gap-1 mt-5 ml-2">
                <span>View All Products</span><ArrowRight size={13} />
              </Link>
            </div>
            ...
            <div className="bg-gradient-to-br from-[#FF6B9D] to-[#FF3F6C] rounded-2xl p-6 relative overflow-hidden shadow-xl text-white">
              ...
              <div className="absolute bottom-2 right-3 text-4xl opacity-60 rotate-12 pointer-events-none select-none">🏋️</div>
              <div className="relative z-10">
                <h3 className="font-bold text-lg">Love Fitness?</h3>
                <p className="text-white/80 text-xs mt-2 leading-relaxed font-semibold">Get the best fitness deals straight to your inbox.</p>
                ...
                {newsAlert && <div className="mt-3 text-[11px] bg-white/15 border border-white/20 rounded-xl p-2.5 font-bold">✓ You're subscribed to fitness deals!</div>}
              </div>
            </div>
          </aside>
        </div>
