...
import {
  ...
  ArrowRight, Sparkles, Package, RefreshCw, Lock, ExternalLink
} from 'lucide-react';
...
const PRODUCTS = [
  {
    id: 'minimalist-kit',
    name: "Minimalist Anti-Pigmentation Kit, Face Wash, Serum & Sunscreen SPF 50",
    brand: 'Minimalist',
    image: 'https://res.cloudinary.com/dqjlffxja/image/upload/v1783162150/51VX_QeZjRL._SY450__mhxhvl.jpg',
    dealPrice: 1147,
    originalPrice: 1299,
    discountPercent: 12,
    affiliateUrl: 'https://amzn.to/4oRAEV3',
    slug: '/products/minimalist-anti-pigmentation-kit',
    isPopular: true,
  },
];
...
export default function BeautyCategory() {
  ...
  const processed = useMemo(() => {
    let r = [...PRODUCTS];
    if (isFilterActive) r = r.filter(p => p.discountPercent >= 10);
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-12 gap-10 items-center text-left">
          ...
          <div className="md:col-span-5 flex items-center justify-center relative w-full">
            <div className="relative w-full h-[280px] rounded-3xl overflow-hidden shadow-2xl">
              <img src="https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?w=800&q=80" alt="Beauty" className="w-full h-full object-cover rounded-3xl hover:scale-105 transition-transform duration-700" referrerPolicy="no-referrer" />
              ...
              <div className="absolute bottom-4 right-4 bg-white rounded-2xl p-5 shadow-2xl min-w-[180px] border border-[#E8E8F0]/40 z-10 text-left">
                ...
                <div className="text-xl font-black text-[#1A1A2E] tracking-tight mt-0.5">BEAUTY</div>
                <div className="text-xs text-[#4A4A6A] mt-1.5 font-semibold">Up to</div>
                <div className="text-3xl font-black text-[#22C55E] leading-none mt-0.5 tracking-tight">12% OFF</div>
                <a href="https://amzn.to/4oRAEV3" target="_blank" rel="noopener noreferrer nofollow sponsored" className="bg-[#FF5722] hover:bg-orange-600 text-white px-5 py-2.5 rounded-full text-xs font-bold mt-4 block w-full text-center transition-all shadow-md">Shop Now</a>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-8 items-start">
          <div className="flex-1 w-full text-left">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3 pb-4 border-b border-[#E8E8F0]">
              <div>
                <h2 className="font-bold text-xl text-[#1A1A2E] leading-none">Beauty Products ({PRODUCTS.length})</h2>
          ...
          <aside className="w-full lg:w-72 flex-shrink-0 space-y-5 lg:sticky lg:top-24 text-left">
              ...
              <div className="space-y-3.5">
                {PRODUCTS.map((p, i) => (
                  <div key={p.id} className="flex items-center gap-3 group">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 ${i === 0 ? 'bg-[#FFD700] text-[#1A1A2E]' : 'bg-[#F0EEFF] text-[#5B4FBE]'}`}>{i + 1}</div>
            ...
            <div className="bg-gradient-to-br from-[#FF6B9D] to-[#FF3F6C] rounded-2xl p-6 relative overflow-hidden shadow-xl text-white">
              ...
              <div className="relative z-10">
                <h3 className="font-bold text-lg">Love Beauty?</h3>
                <p className="text-white/80 text-xs mt-2 leading-relaxed font-semibold">Get the best beauty deals straight to your inbox.</p>
                ...
                {newsAlert && <div className="mt-3 text-[11px] bg-white/15 border border-white/20 rounded-xl p-2.5 font-bold">✓ You're subscribed to beauty deals!</div>}
              </div>
            </div>
          </aside>
        </div>
